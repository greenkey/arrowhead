...
<body>
  ...
  private async generateIndexPage(vaultData: VaultData, outputPath: string): Promise<void> {
    ...
    // Generate list of recent posts (limit to 10, but show all if less than 10)
    const recentPosts = sortedFiles.slice(0, Math.min(10, sortedFiles.length));
    
    // Generate list of recent posts
    const recentPostsContent = recentPosts.map(file => {
      const url = "/" + this.getOutputPath(this.pathToUrl(file.path));
      const title = this.getTitle(file);
    ...
    <article class="recent-post">
      ...
      <time datetime="${new Date(file.modified).toISOString()}">${date}</time>
    </article>`;
    }).join("");
    
    // Generate list of all pages (below recent posts)
    const allPagesContent = sortedFiles.map(file => {
      const url = "/" + this.getOutputPath(this.pathToUrl(file.path));
      const title = this.getTitle(file);
      return `<li><a href="${url}">${title}</a></li>`;
    }).join("\n    ");
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta name="description" content="${this.plugin.settings.siteDescription}">
  <title>${this.plugin.settings.siteTitle}</title>
  <link rel="stylesheet" href="/assets/styles.css">
</head>
<body>
  <header class="site-header">
  ...
  <main class="site-content">
    <section class="hero">
      <h1>${this.plugin.settings.siteTitle}</h1>
      <p class="site-description">${this.plugin.settings.siteDescription}</p>
    </section>
...
${recentPostsContent}
      ...
      ${vaultData.files.length > 10 ? `
      <div class="view-all">
        <a href="/sitemap" class="view-all-link">View all posts (${vaultData.files.length})</a>
      </div>` : ''}
    </section>
    
    ${vaultData.files.length <= 5 ? `
